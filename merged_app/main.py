...
class TranscribeThread(QThread):
    ...
    def _whisper_worker_script():
        return (
            ...
            "    model = whisper.load_model(sys.argv[1], device=device)\n"
            "except Exception as e:\n"
            "    import traceback\n"
            "    print(json.dumps({'url':'', 'ok':False, 'error':f'MODEL_LOAD_FAILED: {str(e)[:200]}', 'fsize':0, 'tb':traceback.format_exc()[:300]}, ensure_ascii=False), flush=True)\n"
            "    sys.exit(1)\n"
            "sys.stderr.write('MODEL_READY\\n')\n"
            "sys.stderr.flush()\n"
            "import threading\n"
            "def _transcribe_safe(model, path):\n"
            "    r = [None]; err = [None]\n"
            "    def _run():\n"
            "        try:\n"
            "            r[0] = model.transcribe(path, language='zh', fp16=False,\n"
            ...
            "    t.join(timeout=600)\n"
            "    if t.is_alive():\n"
            "        raise TimeoutError('transcribe timeout 600s')\n"
            "    if err[0]:\n"
            "        raise err[0]\n"
            "    return r[0]\n"
